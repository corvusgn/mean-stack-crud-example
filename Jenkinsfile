    ...
    if ( "${branchName}" != "refs/heads/master" ) {
        stage('Upgrade chart') {
            ...
            withCredentials([file(credentialsId: params.googleKuberDeployer, variable: 'GOOGLE_KUBE_CREDS')]) {
                ...
                sh "helm status ${env.releaseName} || helm install -n ${env.releaseName} --namespace ${branchName} . && helm upgrade --set {imageName}.image.tag=${imageTag} ${env.releaseName} --namespace ${branchName} ."
            }
        }
    }
}
