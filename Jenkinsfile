    ...
    if ( "${branchName}" != "master" ) {
        stage('Upgrade chart') {
            ...
            withCredentials([file(credentialsId: params.googleKuberDeployer, variable: 'GOOGLE_KUBE_CREDS')]) {
                ...
                sh "if helm status ${env.releaseName}; then if helm upgrade --dry-run ${env.releaseName} .; then helm upgrade --set ${env.imageName}.image.tag=${imageTag} ${env.releaseName} --namespace ${branchName} --namespace ${branchName} .; fi else helm install -n ${env.releaseName} --namespace ${branchName} .; fi"
